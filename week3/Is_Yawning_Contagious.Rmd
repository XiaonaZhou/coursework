...
s <- c(rep(0,36), rep(1,14))

p_diff <- function() {
  index <- sample(1:50,34)
  treat <- s[index]
  contr <- s[-index]
sum(treat)/34-sum(contr)/16
}
...
```{r estimate-p-val}
num_as_extreme <- sum(p_hat >= obs_diff)
p_value <- num_as_extreme / length(p_hat)
p_value
```
...
No, I failed to reject the null hypothesis. The p-value is `r p_value`, which is much greater than $\alpha$ = 0.05. In other words, we did not find statistically significant evidents to suppor that yawning is contagious
...
p_diff <- function() {
treat <- rbinom(1, 34, p_treat)
control <- rbinom(1, 16, p_control)
treat/34-control/16

}

p_hat <- replicate(1e3, p_diff())
...
Have more participants in control group. The size of the control group is less than half of the treatment group. 